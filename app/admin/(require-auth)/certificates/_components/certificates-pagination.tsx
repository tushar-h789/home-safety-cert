...
import { cn } from "@/lib/utils"; // Ensure this import is correct for your project structure
import { CertificateWithRelation } from "@/types/certificates";

export function CertificatesPagination({
  certificates,
  pagination,
}: {
  certificates: CertificateWithRelation[];
  ...
  return (
    <div className="relative">
      <div className="text-xs text-muted-foreground absolute left-0 top-1/2 -translate-y-1/2">
        Showing <strong>{certificates.length}</strong> of{" "}
        <strong>{pagination.totalCount}</strong> Certificates
      </div>
